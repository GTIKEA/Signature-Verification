...
@app.route('/')
def home():
    return render_template('home.html', css_file=url_for('static', filename='style.css'))



real_pic = os.path.join("C:\\Users\\SAINA\\Desktop\\sign-app\\real")
print(real_pic)
forged_pic = os.path.join("C:\\Users\\SAINA\\Desktop\\sign-app\\forged")


#####Complite Method##########

def run(userid, pathtoimage):
    #code goes here
    import numpy as np
    import os
    import matplotlib
    ...
    genuine_image_paths = real_pic
    forged_image_paths = forged_pic
    ...
    n_input = 9
    train_person_id = userid
    test_image_path = pathtoimage
    ...
    def readCSV(train_path, test_path, type2=False):
        ...
        corr_train = to_categorical(correct,2)      # Converting to one hot
        # Reading test data
        ...
        if not(type2):
            ...
            temp = [elem[0] for elem in df.values]
            correct = np.array(temp)
            corr_test = to_categorical(correct,2)      # Converting to one hot
        if not(type2):
...
def data():
    if request.method == 'POST':
        ...
        signURL = request.form.get('signatureURL')
        # print(request.form.get('userName'))
        #print(run(user,signURL))
        if run(user,signURL) == True:
